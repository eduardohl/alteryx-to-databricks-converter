...
class TestDemoWorkflowsPySpark:
    ...
    @pytest.mark.parametrize("demo_file", sorted(DEMO_DIR.glob("*.yxmd")) if DEMO_DIR.exists() else [], ids=lambda p: p.stem)
...
class TestAdvancedMLPipeline:
    ...
    def test_ml_pipeline_pyspark_has_ml_constructs(self, pyspark_config: ConversionConfig) -> None:
        ...
        # Should have ML-related constructs
        assert "VectorAssembler" in content or "BoostedModel" in content.lower() or "GBT" in content
        assert "NaiveBayes" in content or "naive" in content.lower()
        assert "KMeans" in content or "KCentroids" in content
        assert "PCA" in content or "PrincipalComponents" in content
